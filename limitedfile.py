...
def main():
    ...
    for index, i in enumerate(new_obstacles):
        ...
        polygon_patch = MPolygon(np.array([g, h]).T, closed=True, color="gray")
    ...
    #print(A.edges)
    nx.draw(A, A_networkx_points, with_labels=True, node_size=20, font_size=12)
    plt.show()
    ...
    MG_Adj = np.zeros((A.number_of_nodes(), A.number_of_edges()))

    for i in range(A.number_of_edges()):
        # print (MG_Edges[i][0],MG_Edges[i][1])
        MG_Adj[MG_Edges[i][0]][i] = 1
        MG_Adj[MG_Edges[i][1]][i] = -1

        # for j in ranges (G.number_of_nodes()):
    print(MG_Adj.shape)
    ...
    with open('cords.txt', 'w') as file:
        # Write each row of the matrix to the file, followed by a semicolon
        file.write("[")
        for i, p in enumerate(A_networkx_points.tolist()):
            file.write(" " + (str(p[0])))
        file.write(";\n")
        for i, p in enumerate(A_networkx_points.tolist()):
            file.write(" " + (str(p[1])))
